...
async def run_single_attempt(page, attempt_count, phone_mode, phone_list=None, progress_filename=None):
    ...
    try:
        ...
        # --- Cek Konfirmasi ---
        print("Mengecek tulisan 'Terima Kasih' di halaman...")
        try:
            # Coba tunggu locator teks "terima kasih" (case-insensitive /i)
            # Ini akan mencari "terima kasih", "Terima kasih", "TERIMA KASIH", dll.
            success_locator = page.locator("text=/terima kasih/i")
            
            # Tunggu hingga 10 detik sampai teks itu muncul
            await success_locator.wait_for(timeout=10000)
            
            # --- JIKA TEKS DITEMUKAN (SUKSES) ---
            print(f"✅ SUCCESS (Tulisan 'Terima Kasih' ditemukan di halaman)")
            ...
            # --- JIKA TEKS TIDAK DITEMUKAN (GAGAL) ---
             print(f"❌ GAGAL: Submit diklik, tapi tidak ada tulisan 'Terima Kasih'. (Captcha mungkin salah?)")
             ...
             raise Exception("Submit gagal, tidak ada konfirmasi 'Terima Kasih' atau Captcha salah")
