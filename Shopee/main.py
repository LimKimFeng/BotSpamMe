...
def update_harga_shopee(input_file, output_file, persentase_kenaikan):
    ...
    try:
        ...
        if file_extension == '.csv':
            ...
            # Gunakan engine 'openpyxl' untuk file Excel modern (.xlsx)
            df = pd.read_excel(input_file, header=None, keep_default_na=False, dtype=str, engine='openpyxl')
        ...
        print(f"ERROR: Library 'openpyxl' dibutuhkan untuk membaca file .xlsx.")
    ...
    for i in range(HEADER_ROWS, len(df)):
        try:
            ...
            if pd.notna(current_price_numeric):
                ...
                # 3. Hitung harga baru dan bulatkan ke atas (ceil)
                new_price = math.ceil(current_price * faktor_pengali)
                
                # 4. Update nilai di DataFrame sebagai string
                df.iat[i, PRICE_COLUMN_INDEX] = str(new_price)
                rows_processed += 1
