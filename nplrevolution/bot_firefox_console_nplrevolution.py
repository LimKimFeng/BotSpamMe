...
# ===== Helpers =====
def fill_text_with_js(driver, value, field_id, log_message):
    js_code = f"""
    document.getElementById('{field_id}').value = '{value}';
    console.log('{log_message}: {value}');
    """
    driver.execute_script(js_code)
    time.sleep(2)  # Give some time for the console log and browser to process
...
def do_order_once(driver, target, timeout=90):
    ...
    # **Menunggu 30 detik sebelum mengisi form**
    print("Menunggu 30 detik...")
    time.sleep(120)  # Tunggu selama 120 detik
    ...
    print("Filling Name...")
    fill_text_with_js(driver, data["name"], 'user_name', 'Nama Lengkap terisi')
    
    # Mengisi Email
    print("Filling Email...")
    fill_text_with_js(driver, data["email"], 'user_email', 'Email terisi')

    # Mengisi Nomor WhatsApp
    print("Filling Phone...")
    fill_text_with_js(driver, data["phone"], 'user_phone', 'Nomor WhatsApp terisi')
