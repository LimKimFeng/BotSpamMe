...
def do_order_once(browser, target, timeout_sec=90):
    ...
    try:
        ...
        # --- PERUBAHAN: Waktu tunggu 30 detik untuk indikator sukses ---
        print("Menunggu halaman sukses (indikator: 'expiry-date', timeout 30 detik)...")
        # Keyword/selektor p[data-testid="expiry-date"] dikonfirmasi dari HTML baru
        page.wait_for_selector('p[data-testid="expiry-date"]', timeout=30000) # Diubah dari 20000
        
        print("✅ Checkout berhasil terdeteksi!")
        return {"status": "Success", "data": data, "error": None}

    except PlaywrightTimeoutError as e:
        # Perbarui pesan error untuk mencerminkan timeout baru
        error_msg = "Timeout 30s menunggu halaman sukses" if "expiry-date" in str(e) else f"Timeout: Gagal load form awal. ({str(e)})"
        print(f"❌ {error_msg}")
...
# * **Log Persisten:** Skrip sekarang akan *melanjutkan* log jika file `order_log.csv` sudah ada. Ini berarti Anda bisa menghentikan dan memulai ulang skrip tanpa kehilangan riwayat.
# * **Perbaikan Pembacaan CSV:** Saya memperbaiki generator HTML agar lebih aman saat membaca file CSV, seandainya ada baris yang tidak lengkap.

# Jalankan `order_bot_v4.py` dan biarkan bekerja. Saat Anda hentikan, `laporan_order.html` akan berisi rekap lengkapnya.
# **Instruksi:**

# 1.  Simpan kedua file (`order_bot_v3.py` dan `laporan_order.html`) di folder yang sama.
# 2.  Jalankan skrip Python: `python order_bot_v3.py` (atau `python order_bot_v3.py --headless`).
# 3.  Biarkan skrip berjalan. File `order_log.csv` akan dibuat dan diisi.
# 4.  Jika Anda membuka `laporan_order.html` *saat skrip berjalan*, Anda hanya akan melihat pesan "Menunggu...".
# 5.  Setelah Anda menghentikan skrip (tekan `Ctrl+C`), skrip akan otomatis memproses `order_log.csv` dan **menimpa** file `laporan_order.html` dengan laporan hasil akhirnya.
# 6.  Buka (atau refresh) file `laporan_order.html` di browser Anda untuk melihat ringkasan dan semua lognya.